...
export function TokenDistribution({ id }: Props) {
  return (
    <>
      <style jsx>{`
        .icon {
          filter: drop-shadow(0px 20px 60px rgba(47, 69, 197, 0.4));
        }
      `}</style>
      <div className='flex flex-col items-center pt-14 lg:pt-20' id={id}>
        ...
        <div className='relative flex flex-col items-center justify-center w-full pt-10 lg:pt-20 bg-trueGray-50'>
          <div
            className='absolute top-0 z-10 flex items-center justify-center w-32 h-32 transform -translate-y-1/2 rounded-full lg:w-60 lg:h-60 bg-primaryMain icon'>
...
function Item({ children, index }: ItemProps) {
  return (
    <div className='flex items-center lg:gap-x-5 gap-x-3'>
      <div
        className='flex items-center justify-center flex-none text-base font-bold leading-none text-white rounded-full select-none w-9 h-9 lg:w-12 lg:h-12 lg:text-2xl bg-primaryMain'>
        {index + 1}
      </div>
